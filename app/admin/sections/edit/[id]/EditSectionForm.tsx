...
export default function EditSectionForm({ section, products }: { section: StoreSection; products: Product[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const updatedSection: StoreSection = {
      id: section.id,
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">تعديل القسم: {section.title.ar}</h2>
          <p className="text-gray-500 mt-1">قم بتعديل إعدادات وترتيب هذا القسم المعروض في الصفحة الرئيسية.</p>
        </div>
        <Link
          href="/admin/sections"
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-100 shadow-sm p-8 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input required name="title_ar" type="text" defaultValue={section.title.ar} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-[#f4d7b1] focus:border-[#201711] outline-none transition text-right" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">عنوان القسم (إنجليزي) *</label>
            <input required name="title_en" type="text" defaultValue={section.title.en} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-[#f4d7b1] focus:border-[#201711] outline-none transition text-left" dir="ltr" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <select required name="type" defaultValue={section.type} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-[#f4d7b1] focus:border-[#201711] outline-none transition">
          ...
          <div>
            ...
            <input required name="order" type="number" defaultValue={section.order} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-[#f4d7b1] focus:border-[#201711] outline-none transition" />
          </div>
        </div>
